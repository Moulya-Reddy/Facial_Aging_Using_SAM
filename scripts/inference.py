...
from dataset.augmentations import AgeTransformer
    ...
    for age_transformer in age_transformers:
        ...
        for input_batch in tqdm(dataloader):
            if global_i >= opts.n_images:
                break
            with torch.no_grad():
                input_age_batch = [age_transformer(img.cpu()).to('cpu') for img in input_batch]
                input_age_batch = torch.stack(input_age_batch)
                input_cpu = input_age_batch.to('cpu').float()
                tic = time.time()
                result_batch = run_on_batch(input_cpu, net, opts)
                toc = time.time()
